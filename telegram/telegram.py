...
class TelegramCommand(models.Model):
    ...
    def odoo_listener(self, message, bot):
        ...
        with openerp.api.Environment.manage(), db.cursor() as cr:
            ...
            elif bus_message['action'] == 'manage_subscriptions':
                self.manage_subscriptions(bus_message, bot)
            else:
    ...
    @api.model
    def telegram_update_cache_event(self):
    ...
    @api.model
    def telegram_manage_subscriptions_event(self):
        # Called by ir.actions.server
        ...
        if len(found_subscription_commands):
            message = {'action': 'manage_subscriptions', 'active_id': context['active_id'], 'update_cache': False, 'model': context['active_model'], 'found_commands_ids': found_subscription_commands.ids}
            self.env['telegram.bus'].sendone('telegram_channel', message)
    ...
    def manage_subscriptions(self, bus_message, bot):
        _logger.debug('manage_subscriptions() - command from bus')
        for command_id in bus_message['found_commands_ids']:
            command = self.env['telegram.command'].browse(command_id)
